...
export function KpiCard({ label, value, sublabel }: KpiCardProps) {
  return (
    ...
    >
      ...
      <p className="mt-2.5 text-[28px] font-semibold leading-none tabular-nums tracking-tight" style={{ color: 'var(--text)' }}>
        {value}
      </p>
      {sublabel && <div className="mt-2 text-sm" style={{ color: 'var(--text-secondary)' }}>{sublabel}</div>}
    </div>
  );
}
